...
export const DriverRoutePublishForm = ({
  ...
  return (
    <form onSubmit={manejarPublicarTrayecto} className="space-y-4">
      {/* 1. SELECCIÓN DE SEDE UNIVERSITARIA */}
      <section
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-black">Sede Universitaria de Destino/Origen</h3>
        </div>

        <FormSelect
          value={sedeSeleccionada}
          onChange={(e) => setSedeSeleccionada(e.target.value)}
          options={sedesInstitucion.map((sede) => ({
            value: sede.name,
      ...
      {/* 2. PUNTO PERSONALIZADO (BARRIO O CORREDOR EN EL AMB) */}
      <section
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-black">
              {sentidoViaje === 'hacia_campus' ? '¿Desde dónde sales?' : '¿Hacia dónde te diriges?'}
            </h3>
          </div>
          <button
            type="button"
      ...
      {/* 3. PARÁMETROS DEL VIAJE: HORA, CUPOS Y TARIFA */}
      <section
        className={`rounded-3xl p-4 border shadow-sm space-y-3 transition-colors ${
          isDark ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'
        }`}
